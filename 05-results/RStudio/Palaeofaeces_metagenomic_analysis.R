...
#  stat_ellipse(aes(group = Site), level = 0.90) +
  labs(x = "PC1 (16%)", #NOTE: put in PC values
       y = "PC2 (14%)", #NOTE: put in PC values
...
ggsave("PCA_CLR_decontam_t0.75_fraction0.001_2024-standard_minimizer.png", 
       width = 12, height = 8)
...
species_list <- c("Lactococcus petauri", "Lactococcus garvieae", "Lactococcus cremoris",
...
otu_fish <- otu_fish[rownames(otu_fish) %in% species_list, ]
...
otu_top20 <- merged[top_taxa, ]

# Clr transform for better visualization
otu_top20_clr <- microbiome::transform(otu_top20, "clr", pseudocount = 0)
...
#ggsave("heatmap_top100_species_all_samples_overall_clustering_filter_march2025.png", plot = p, width = 24, height = 24)  
