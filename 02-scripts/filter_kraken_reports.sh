...
do
bracken -d $DB -i reports/${sample}_kraken_${prefix}_conf0.50_filtered.txt \
        -o bracken_genus/${sample}_minimizer.bracken -l G
python ../KrakenTools/filter_bracken.out.py -i bracken_genus/${sample}_minimizer.bracken \
        --exclude 9605 -o bracken_genus/${sample}_minimizer_filtered.bracken
done
...
python combine_bracken_outputs.py \
--files bracken_genus/*minimizer_filtered.bracken \
-o palaeofaeces_combined_${prefix}_minimizer_bracken_genus.bracken

python tsv_to_csv.py < palaeofaeces_combined_${prefix}_minimizer_bracken_genus.bracken \
        > palaeofaeces_combined_${prefix}_minimizer_bracken_genus.csv
python tsv_to_csv.py < negatives_combined_${prefix}_minimizer_bracken_species.bracken \
        > negatives_combined_${prefix}_minimizer_bracken_species.csv
