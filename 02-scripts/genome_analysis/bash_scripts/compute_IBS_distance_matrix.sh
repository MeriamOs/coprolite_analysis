...
#Initially I kept the ancient dingoes in list - but ended up removing these because got an error
...
###############################
### KURI BOOTSTRAP OUTGROUP ###
###############################

plink --bfile merged_kuri_souilmi --make-bed --dog --out kuri_outgroup --maf 0.2 \
	--snps-only --biallelic-only --missing-genotype N --geno 0.8 --keep kuri_outgroup_to_keep.txt
#239,577 variants remain using SiberianHusky01 as an outgroup
...
#conda create -n booster booster=0.1.2 --solver=classic
conda activate /nesi/nobackup/uoo02328/meriam/conda_environments/booster
